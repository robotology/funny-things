...
scene_0() {
    speak "molto bene grazie, sono in mezzo ai bambini e sono felice!"
    go_home
}

scene_1() {
    speak "Si, iCub infatti vuol dire cucciolo di robot, ho le dimensioni e la forma di un bambino, e come i bambini posso imparare cose nuove"
}


scene_2() {
    speak "posso riconoscere gli oggetti intorno a me ed afferrarli, posso usarli per svolgere alcuni semplici compiti. Da poco ho imparato a stare in equilibrio senza cadere e presto imparerò a camminare proprio come voi. E poi sono l’unico robot al mondo dotato di una pelle sensibile che mi permette di sentire se vengo toccato o spinto."
    # breathers "stop"
...
scene_3() {
    speak "Si, però grazie ai ricercatori dell’Istituto Italiano di Tecnologia ho oltre 30 fratelli sparsi per il mondo dagli Stati Uniti al Giappone su cui lavorano scienziati provenienti da ogni parte del mondo."
}

scene_4() {
    speak "Credo che sia fantastica! A me piace la scienza e credo che questa iniziativa possa essere utile a tutti i bambini sia robot che umani." 
#    sleep 3.0 
#    speak "Tutti attendono" 
}

scene_5() {
    speak "Potrò aiutarvi nei lavori domestici o in altre attività delicate per cui avrete bisogno di supporto, come per esempio l’ assistenza degli anziani o a persone disabili" 
}


scene_6() {
    speak "Grazie a te e un saluto a tutti dalla città dei bambini di Genova"
}
...
scene_pause() 
{
    ...
    scene_0
    # Anche tu in fondo sei un bambino, giusto?
    read -p "waiting for enter"
    scene_1
    # E cosa sai fare?
    read -p "waiting for enter"
    scene_2
    # Quindi sei un robot unico al mondo?
    read -p "waiting for enter"
    scene_3
    # Cosa ne pensi di questa iniziativa della città dei bambini?
    read -p "waiting for enter"
    scene_4
    # E tu come potrai esserci utile in futuro?
    read -p "waiting for enter"
    scene_5
    # Non vediamo l’ora! Buon lavoro e Grazie iCub per il tuo tempo, a presto!
    read -p "waiting for enter"
    scene_6
}
